...
const resultHomepage = document.getElementById("homepage-result");

//request fetch para API fake 
...
function displayResults(results) {
  resultHomepage.classList.add('hidden')
...
searchInput.addEventListener("input", function () {
  ...
  if (searchTerm === "") {
    ...
    resultHomepage.classList.remove("hidden");
    return;
  }
  //para chamar o request via fetch usando fake api
  // requestApi(searchTerm);

  let resultsArr = artists.filter((data) => {
    if(JSON.stringify(data).toLocaleLowerCase().indexOf(searchTerm) !== -1) {
      return data
    }
  });
